...
def plot_candlestick(data):
    ...
    # Add lines for first purchase date and price with a black border
    purchase_date = '4th March 2025'
    purchase_price = 929.2

    # Skyblue dashed color line for 'Purchase Date'
    skyblue_line = ax.axvline(x=mpl_dates.date2num(pd.to_datetime(purchase_date)), color='skyblue', linewidth=4,
    linestyle='dotted', label=f'1st Purchase Date: {purchase_date}', zorder=5)
    skyblue_line.set_path_effects([path_effects.Stroke(linewidth=6, foreground='black'), path_effects.Normal()])

    # Skyblue solid color line for 'Purchase Price'
    skyblue_line = ax.axhline(y=purchase_price, color='skyblue', linewidth=4, label=f'1st Purchase Price: {purchase_price}',
    zorder=5)
    skyblue_line.set_path_effects([path_effects.Stroke(linewidth=6, foreground='black'), path_effects.Normal()])

    # Add legend below the x-axis and center it with bold text and a black border around the lines in the legend
    ax.legend(
    loc='upper center',
    bbox_to_anchor=(0.48, -0.13),
    fontsize=10,
    edgecolor='black',
    framealpha=1,
    prop={'weight': 'bold'})  # Set text in the legend to bold
...
root = tk.Tk()
root.title("HSBA.L Japanese Candlestick Chart - One Purchase")
